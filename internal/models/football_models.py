...
class TeamStats(BaseModel):
    ...
    def get_team_points(self, team_tactic: TeamTactics):
        if team_tactic == TeamTactics.Possesion:
            pass_acc_score = 10 if self.pass_accuracy > 80 else -5
            pass_score = 10 if self.total_passes > 350 else -5
            lost_ball_score = 10 if self.dispossesed < 7 else -5
            return pass_acc_score + pass_score + lost_ball_score
        elif team_tactic == TeamTactics.Defensive:
            tackles = 10 if self.tackles > 20 else -5
            interceptions = 10 if self.interceptions > 10 else -5
            goals_conceded = 10 if self.goals_conceded < 10 else -5
            return tackles + interceptions + goals_conceded
        elif team_tactic == TeamTactics.Offensive:
            goals_points = 10 if  self.goals + self.assists > 6 else -5
            key_passes = 10 if self.key_passes > 10 else -5
            goals_diff_points = 10 if self.goals_conceded < (self.goals + self.assists) else -5
